...
function Vector3D.GetLength(x, y, z)
    return sqrt(Vector3D.GetLengthSquared(x, y, z))
end
...
function Vector3D.Normalize(x, y, z)
    return Vector3D.DivideBy(Vector3D.GetLength(x, y, z), x, y, z)
end
...
function Vector3D.CalculateYawPitchFromNormalVector(vector)
    return Vector3D.CalculateYawPitchFromNormal(vector:GetXYZ())
end
...
function Vector3D.CreateNormalVectorFromYawPitch(yawRadians, pitchRadians)
    return Vector3D.Create(Vector3D.CalculateNormalFromYawPitch(yawRadians, pitchRadians))
end

-- Create the mixin
--- @class Vector3DMixin : Vector3D, ZO_InitializingObject
...
local Vector3DMixin = ZO_InitializingObject:Subclass()
...
function Vector3D.Create(x, y, z)
    --- @class Vector3DMixin
    local vector = setmetatable({}, Vector3DMixin)
...
--- @param y number The y component
--- @param z number The z component
function Vector3DMixin:Initialize(x, y, z)
    self:SetXYZ(x, y, z)
end
...
function Vector3DMixin:ScaleBy(scalar)
    self:SetXYZ(Vector3D.ScaleBy(scalar, self:GetXYZ()))
    return self
end
...
function Vector3DMixin:DivideBy(scalar)
    self:SetXYZ(Vector3D.DivideBy(scalar, self:GetXYZ()))
    return self
end
...
function Vector3DMixin:Add(other)
    self:SetXYZ(Vector3D.Add(self.x, self.y, self.z, other:GetXYZ()))
    return self
end
...
function Vector3DMixin:Subtract(other)
    self:SetXYZ(Vector3D.Subtract(self.x, self.y, self.z, other:GetXYZ()))
    return self
end
...
function Vector3DMixin:Cross(other)
    self:SetXYZ(Vector3D.Cross(self.x, self.y, self.z, other:GetXYZ()))
    return self
end
...
function Vector3DMixin:Dot(other)
    return Vector3D.Dot(self.x, self.y, self.z, other:GetXYZ())
end

--- Gets the squared length (magnitude) of the vector
--- @return number lengthSquared The squared length of the vector
function Vector3DMixin:GetLengthSquared()
    return Vector3D.GetLengthSquared(self:GetXYZ())
end

--- Gets the length (magnitude) of the vector
--- @return number length The length of the vector
function Vector3DMixin:GetLength()
    return Vector3D.GetLength(self:GetXYZ())
end
...
function Vector3DMixin:Normalize()
    self:SetXYZ(Vector3D.Normalize(self:GetXYZ()))
    return self
end
...
function Vector3DMixin:Clone()
    return Vector3D.Create(self:GetXYZ())
end

---@type Vector3D
LibVectorMath.Vector3D = Vector3D
---@type Vector3DMixin
LibVectorMath.Vector3DMixin = Vector3DMixin
